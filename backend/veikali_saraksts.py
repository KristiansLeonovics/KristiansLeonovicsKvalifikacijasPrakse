...
def izdzest_veikalus():
    if request.method == 'POST' and 'nosaukums' in request.form:
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        nosaukums = request.form['nosaukums']
        cursor.execute("DELETE FROM veikali WHERE nosaukums = %s", (nosaukums, ))
        mysql.connection.commit()
        return redirect(url_for('veikalu_saraksts'))
    return redirect(url_for('veikalu_saraksts'))

def pievienot_veikalus():
    if request.method == 'POST' and 'nosaukums' in request.form and 'atrasanasvieta' in request.form and 'darbalaiks' in request.form and 'WEBadrese' in request.form  and 'telefonanumurs' in request.form:
        ...
        cursor.execute("INSERT INTO veikali (nosaukums, atrasanasvieta, darbalaiks, WEBadrese, telefonanumurs) VALUES (%s, %s, %s, %s, %s)", (nosaukums, atrasanasvieta, darbalaiks, WEBadrese, telefonanumurs,))
        mysql.connection.commit()
        return redirect(url_for('veikalu_saraksts'))
    return redirect(url_for('veikalu_saraksts'))
...
def rediget_veikali():
    if request.method == 'POST' and 'nosaukums' in request.form and 'jaunais_nosaukums' in request.form and 'atrasanasvieta' in request.form and 'darbalaiks' in request.form and 'WEBadrese' in request.form and 'telefonanumurs' in request.form:
        ...
        cursor.execute("SELECT * FROM veikali WHERE nosaukums = %s", (nosaukums,))
        ...
        if datubazes_dati:
            ...
            cursor.execute("UPDATE veikali SET nosaukums = %s, atrasanasvieta = %s, darbalaiks = %s, WEBadrese = %s, telefonanumurs = %s WHERE nosaukums = %s", (datubazes_dati['nosaukums'], datubazes_dati['atrasanasvieta'], datubazes_dati['darbalaiks'], datubazes_dati['WEBadrese'], datubazes_dati['telefonanumurs'], nosaukums,))
            mysql.connection.commit()
